...
async function sendUpdateAlert(subscriberEmails, syncResult) {
  const t = createTransporter();
  ...
  const body = `
    ...
    <p style="margin:0 0 10px;font-size:14px;color:#555;line-height:1.65;">
      El <strong>Mapa de Ayudas de la Comunidad de Madrid</strong> ha sido actualizado
      el <strong>${date}</strong>. Puede que haya nuevas convocatorias abiertas o cambios
      en las existentes.
    </p>
    <div style="background:#f8f9fa;border-left:3px solid #e67e22;padding:12px 16px;margin:16px 0;border-radius:0 4px 4px 0;">
      <p style="margin:0;font-size:12px;font-family:monospace;color:#95a5a6;">
        Fuente: ${syncResult.urlFound||'—'}
      </p>
    </div>
    ${btn(HOME_URL, 'Ver el Mapa de Ayudas →')}`;

  const subject = `[Mapa de Ayudas] Actualización disponible — ${date}`;
